...
K.prototype.subscribe=K.prototype.subscribe;K.prototype.unsubscribeByKey=K.prototype.Ha;K.prototype.publish=K.prototype.ta;K.prototype.clear=K.prototype.clear;A("ytPubsubPubsubInstance",gq);A("ytPubsubPubsubTopicToKeys",iq);A("ytPubsubPubsubIsSynchronous",jq);A("ytPubsubPubsubSubscribedKeys",hq);function qq(){this.store={}}
qq.prototype.storePayload=function(a,b){a=rq(a);this.store[a]?this.store[a].push(b):this.store[a]=[b];return a};
qq.prototype.extractMatchingEntries=function(a){a=sq(this,a);for(var b=[],c=0;c<a.length;c++)b.push.apply(b,ia(this.store[a[c]])),delete this.store[a[c]];return b};
qq.prototype.getSequenceCount=function(a){a=sq(this,a);for(var b=0,c=0;c<a.length;c++)b+=this.store[a[c]].length||0;return b};
function sq(a,b){a=Object.keys(a.store)||[];if(1>=a.length&&rq(b)===a[0])return a;for(var c=[],d=0;d<a.length;d++){var e=a[d].split("/");if(tq(b.auth,e[0])){var f=b.isJspb;tq(void 0===f?"undefined":f?"true":"false",e[1])&&tq(b.cttAuthInfo,e[2])&&c.push(a[d])}}return c}
...
":"+la);if("IGNORED"===a.level)var fa=0;else a:{for(var xe=xr(),ye=p(xe.oa),od=ye.next();!od.done;od=ye.next()){var pa=od.value;if(H.message&&H.message.match(pa.xr)){fa=pa.weight;break a}}for(var cp=p(xe.ma),Nj=cp.next();!Nj.done;Nj=cp.next()){var dp=Nj.value;if(dp.callback(H)){fa=dp.weight;break a}}fa=1}H.sampleWeight=fa;for(var ep=p(sr),Oj=ep.next();!Oj.done;Oj=ep.next()){var Pj=Oj.value;if(Pj.jb[H.name])for(var fp=p(Pj.jb[H.name]),Qj=fp.next();!Qj.done;Qj=fp.next()){var gp=Qj.value,Ig=H.message.match(gp.regexp);
if(Ig){H.params["params.error.original"]=Ig[0];for(var Rj=gp.groups,hp={},pd=0;pd<Rj.length;pd++)hp[Rj[pd]]=Ig[pd+1],H.params["params.error."+Rj[pd]]=Ig[pd+1];H.message=Pj.Bb(hp);break}}}H.params||(H.params={});var ip=xr();H.params["params.errorServiceSignature"]="msg="+ip.oa.length+"&cb="+ip.ma.length;H.params["params.serviceWorker"]="false";z.document&&z.document.querySelectorAll&&(H.params["params.fscripts"]=String(document.querySelectorAll("script:not([nonce])").length));Db("sample").constructor!==
Bb&&(H.params["params.fconst"]="true");window.yterr&&"function"===typeof window.yterr&&window.yterr(H);if(0!==H.sampleWeight&&!Er.has(H.message)){if("ERROR"===h){yr.ta("handleError",H);if(M("record_app_crashed_web")&&0===Hr&&1===H.sampleWeight)if(Hr++,M("errors_via_jspb")){var Sj=new uj;D(Sj,1,1);if(!M("report_client_error_with_app_crash_ks")){var jp=new pj;D(jp,1,H.message);var kp=new qj;G(kp,pj,3,jp);var lp=new rj;G(lp,qj,5,kp);var mp=new sj;G(mp,rj,9,lp);G(Sj,sj,4,mp)}var np=new mk;be(np,uj,20,
nk,Sj);kr(np)}else{var op={appCrashType:"APP_CRASH_TYPE_BREAKPAD"};M("report_client_error_with_app_crash_ks")||(op.systemHealth={crashData:{clientError:{logMessage:{message:H.message}}}});fm("appCrashed",op)}Gr++}else"WARNING"===h&&yr.ta("handleWarning",H);if(M("kevlar_gel_error_routing"))a:{var ze=h;if(M("errors_via_jspb")){if(Mr())var qp=void 0;else{var qd=new mj;D(qd,1,H.stack);H.fileName&&D(qd,4,H.fileName);var Gb=H.lineNumber&&H.lineNumber.split?H.lineNumber.split(":"):[];0!==Gb.length&&(1!==
Gb.length||isNaN(Number(Gb[0]))?2!==Gb.length||isNaN(Number(Gb[0]))||isNaN(Number(Gb[1]))||(D(qd,2,Number(Gb[0])),D(qd,3,Number(Gb[1]))):D(qd,2,Number(Gb[0])));var wc=new pj;D(wc,1,H.message);D(wc,3,H.name);D(wc,6,H.sampleWeight);"ERROR"===ze?D(wc,2,2):"WARNING"===ze?D(wc,2,1):D(wc,2,0);var Tj=new nj;D(Tj,1,!0);be(Tj,mj,3,oj,qd);var Yb=new jj;D(Yb,3,window.location.href);for(var rp=L("FEXP_EXPERIMENTS",[]),Uj=0;Uj<rp.length;Uj++){var Hv=rp[Uj];Ld(Yb);Vd(Yb,5,2,!1).push(Hv)}var Vj=L("LATEST_ECATCHER_SERVICE_TRACKING_PARAMS");
if(!Fk()&&Vj)for(var sp=p(Object.keys(Vj)),xc=sp.next();!xc.done;xc=sp.next()){var tp=xc.value,Wj=new lj;D(Wj,1,tp);D(Wj,2,String(Vj[tp]));de(Yb,4,lj,Wj)}var Xj=H.params;if(Xj){var up=p(Object.keys(Xj));for(xc=up.next();!xc.done;xc=up.next()){var vp=xc.value,Yj=new lj;D(Yj,1,"client."+vp);D(Yj,2,String(Xj[vp]));de(Yb,4,lj,Yj)}}var wp=L("SERVER_NAME"),xp=L("SERVER_VERSION");if(wp&&xp){var Zj=new lj;D(Zj,1,"server.name");D(Zj,2,wp);de(Yb,4,lj,Zj);var ak=new lj;D(ak,1,"server.version");D(ak,2,xp);de(Yb,
4,lj,ak)}var Jg=new qj;G(Jg,jj,1,Yb);G(Jg,nj,2,Tj);G(Jg,pj,3,wc);qp=Jg}var yp=qp;if(!yp)break a;var zp=new mk;be(zp,qj,163,nk,yp);kr(zp)}else{if(Mr())var Ap=void 0;else{var Ae={stackTrace:H.stack};H.fileName&&(Ae.filename=H.fileName);var Hb=H.lineNumber&&H.lineNumber.split?H.lineNumber.split(":"):[];0!==Hb.length&&(1!==Hb.length||isNaN(Number(Hb[0]))?2!==Hb.length||isNaN(Number(Hb[0]))||isNaN(Number(Hb[1]))||(Ae.lineNumber=Number(Hb[0]),Ae.columnNumber=Number(Hb[1])):Ae.lineNumber=Number(Hb[0]));
var bk={level:"ERROR_LEVEL_UNKNOWN",message:H.message,errorClassName:H.name,sampleWeight:H.sampleWeight};"ERROR"===ze?bk.level="ERROR_LEVEL_ERROR":"WARNING"===ze&&(bk.level="ERROR_LEVEL_WARNNING");var Iv={isObfuscated:!0,browserStackInfo:Ae},rd={pageUrl:window.location.href,kvPairs:[]};L("FEXP_EXPERIMENTS")&&(rd.experimentIds=L("FEXP_EXPERIMENTS"));var ck=L("LATEST_ECATCHER_SERVICE_TRACKING_PARAMS");if(!Fk()&&ck)for(var Bp=p(Object.keys(ck)),yc=Bp.next();!yc.done;yc=Bp.next()){var Cp=yc.value;rd.kvPairs.push({key:Cp,
value:String(ck[Cp])})}var dk=H.params;if(dk){var Dp=p(Object.keys(dk));for(yc=Dp.next();!yc.done;yc=Dp.next()){var Ep=yc.value;rd.kvPairs.push({key:"client."+Ep,value:String(dk[Ep])})}}var Fp=L("SERVER_NAME"),Gp=L("SERVER_VERSION");Fp&&Gp&&(rd.kvPairs.push({key:"server.name",value:Fp}),rd.kvPairs.push({key:"server.version",value:Gp}));Ap={errorMetadata:rd,stackTrace:Iv,logMessage:bk}}var Hp=Ap;if(!Hp)break a;fm("clientError",Hp)}if("ERROR"===ze||M("errors_flush_gel_always_killswitch"))b:if(M("migrate_events_to_ts"))c:{if(M("web_fp_via_jspb")&&
...
H.stack);for(var Ip=p(Object.keys(Be)),ek=Ip.next();!ek.done;ek=Ip.next()){var Jp=ek.value;Zb.postParams["client."+Jp]=Be[Jp]}var fk=L("LATEST_ECATCHER_SERVICE_TRACKING_PARAMS");if(fk)for(var Kp=p(Object.keys(fk)),gk=Kp.next();!gk.done;gk=Kp.next()){var Lp=gk.value;Zb.postParams[Lp]=fk[Lp]}var Mp=L("SERVER_NAME"),Np=L("SERVER_VERSION");Mp&&Np&&(Zb.postParams["server.name"]=Mp,Zb.postParams["server.version"]=Np)}ml(L("ECATCHER_REPORT_HOST","")+"/error_204",Zb)}try{Er.add(H.message)}catch(lx){}Fr++}}}}
